...
/* Three-dot chat actions use a unique internal label, but visitors should see
   only the familiar ellipsis icon. */
...
with st.sidebar:
    ...
    for cid, title in chats:
        ...
        with col2:
            # This is a real actions menu. The old three-dot button deleted a
            # chat immediately, which felt broken and was easy to trigger.
            # Variation selectors keep the widget label unique without
            # exposing the database chat id beside the visible ellipsis.
            invisible_id = "".join(
                chr(0xFE00 + int(digit)) for digit in str(cid)
            )
            with st.popover("⋮" + invisible_id, use_container_width=True):
